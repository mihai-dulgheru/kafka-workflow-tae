...
public class SupplierListener {
	...
	@KafkaListener(topics = "order", groupId = "workflow")
	public void onReceive(String data, @Header(KafkaHeaders.RECEIVED_TOPIC) String topic,
			@Header(KafkaHeaders.RECEIVED_PARTITION) int partition, @Header(KafkaHeaders.OFFSET) int offset)
			throws TransformerFactoryConfigurationError, IOException, TransformerException, InterruptedException,
			ExecutionException {
		LOGGER.info("topic = {}, partition = {}, offset = {}", topic, partition, offset);
		switch (topic) {
		case "order":
			inbox.write("supplier", "order", data);
			byte[] result = transformer.transform("supplier", "order2invoice", data.getBytes());
